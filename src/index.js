...
function skipDefault(eventName, target) {
    return target.addEventListener(eventName, event => event.preventDefault());
}
...
function delegate(target, fn) {
    for (let button of [...target.getElementsByTagName('button')]) {
...
function once(target, fn) {
    ...
    // или так, но тогда нужна проверка, поддерживает ли браузер once:
